...
const monthDiv04 = document.querySelector('.clock__month--04');
...
function setDate() {
	...
	const month = new Intl.DateTimeFormat('en', { month: 'long'}).format(now).split('.')[0].toUpperCase();
	monthDiv01.innerText = month[0];
	monthDiv02.innerText = month[1];
	monthDiv03.innerText = month[2];
	monthDiv04.innerText = month[3];
	
	const dayLetters = new Intl.DateTimeFormat('en', { weekday: 'short'}).format(now).split('.')[0].toUpperCase();
